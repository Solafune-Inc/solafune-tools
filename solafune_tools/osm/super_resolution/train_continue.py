...
def train(using_own_dataset : bool = False,  gpus: Union[int, str, List] = 1, 
          debug: bool = False,  strategy: Union[str, bool] = False, 
    ...
    for fold in cfg.trn_folds:
        ...
        if os.path.isfile(os.path.join(models_dir, (cfg.ckpt_stem + f"_fold{fold}.ckpt"))):
            model = SRModel.load_from_checkpoint(os.path.join(models_dir, (cfg.ckpt_stem + f"_fold{fold}.ckpt")), cfg=cfg, fold=fold)
        else:
            print(f"{os.path.join(models_dir, (cfg.ckpt_stem + f'_fold{fold}.ckpt'))} is not exist, skipping training for this checkpoint fold")
            continue
