...
def train(using_own_dataset : bool = False, gpus: Union[int, str, List] = 1, debug: bool = False,  strategy: Union[str, bool] = False):
    ...
    cfg = Box({k: v for k, v in dict(vars(CFG(debug=debug, datasets_dir=dataset.datasets_dir,
                                              batch_size=1, grad_clip=25.0,
                                              learning_rate=3e-5, augmentation_end_epoch=26,
                                              cutmix_end_epoch=20, extra_dataset=True,
                                              extra_dataset_end_epoch=14, train_loader_workers=2,
    ...
    train_nerima_csv_path = cfg.DATA_ROOT / "train_nerima.csv"

    if preprocess_json[os.path.basename(__file__).split(".")[0]] == 0:
        print(f"Rank 0: Preparing data and creating train.csv...")
        if train_csv_path.exists() and train_nerima_csv_path.exists():
            os.remove(train_csv_path) # type: ignore
            os.remove(train_nerima_csv_path) # type: ignore
        ...
        df = pd.merge(
            df,
            pd.read_csv(train_csv_path)[["path_high", "fold"]],
            how="left",
            on="path_high",
        )
        ...
        df.to_csv(train_nerima_csv_path, index=False)
