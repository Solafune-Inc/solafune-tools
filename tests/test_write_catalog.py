...
@pytest.fixture()
def get_catalog():
    random_dir_name = ''.join(random.choices(string.ascii_lowercase + string.digits, k=6)) 
    solafune_tools.settings.set_data_directory(random_dir_name)
    parq_loc = solafune_tools.image_fetcher._make_parquet_filename(start_date="2023-05-01", end_date="2023-08-01", aoi_geometry_file="data/geojson/sample.geojson")
